 ...
 * http://code.google.com/p/php-websocket-server/wiki/Scripting WebSocket
 * Protocol 07 -
 * http://tools.ietf.org/html/draft-ietf-hybi-thewebsocketprotocol-07 -
 * Supported by Firefox 6 (30/08/2011) Whilst a big effort is made to follow the
 * protocol documentation, the current script version may unknowingly differ.
...
class Pws_Server {
	...
	// maximum amount of clients that can be connected at one time on the same
	// IP v4 address
	const WS_MAX_CLIENTS_PER_IP = 15;
	
	// amount of seconds a client has to send data to the server, before a ping
	// request is sent to the client,
	// if the client has not completed the opening handshake, the ping request
	// is skipped and the client connection is closed
	const WS_TIMEOUT_RECV = 10;
	
	// amount of seconds a client has to reply to a ping request, before the
	// client connection is closed
	const WS_TIMEOUT_PONG = 5;
	
	// the maximum length, in bytes, of a frame's payload data (a message
	// consists of 1 or more frames), this is also internally limited to
	// 2,147,479,538
	const WS_MAX_FRAME_PAYLOAD_RECV = 100000;
	
	// the maximum length, in bytes, of a message's payload data, this is also
	// internally limited to 2,147,483,647
	 ...
	 * no incoming frames 2 => integer ReadyState, // between 0 and 3 3 =>
	 * integer LastRecvTime, // set to time() when the client is added 4 =>
	 * int/false PingSentTime, // false when the server is not waiting for a
	 * pong 5 => int/false CloseStatus, // close status that wsOnClose() will be
	 * called with 6 => integer IPv4, // client's IP stored as a signed long,
	 * retrieved from ip2long() 7 => int/false FramePayloadDataLength, // length
	 * of a frame's payload data, reset to false when all frame data has been
	 * read (cannot reset to 0, to allow reading of mask key) 8 => integer
	 * FrameBytesRead, // amount of bytes read for a frame, reset to 0 when all
	 * frame data has been read 9 => string FrameBuffer, // joined onto end as a
	 * frame's data comes in, reset to blank string when all frame data has been
	 * read 10 => integer MessageOpcode, // stored by the first frame for
	 * fragmented messages, default value is 0 11 => integer MessageBufferLength
	 * // the payload data length of MessageBuffer ) $wsRead[ integer ClientID ]
	 * = resource Socket // this one-dimensional array is used for
	 * socket_select() // $wsRead[ 0 ] is the socket listening for incoming
	 * client connections $wsClientCount = integer ClientCount // amount of
	 * clients currently connected $wsClientIPCount[ integer IP ] = integer
	 * ClientCount // amount of clients connected per IP v4 address
	 */
			...
			if ($result === false) {
				...
				foreach ( $changed as $clientID => $socket ) {
					if ($clientID != 0) {
						...
						if ($bytes === false) {
							// error on recv, remove client socket (will check
							// to send close frame)
							...
							// 0 bytes received from client, meaning the client
	...
	function wsStopServer() {
		...
		foreach ( $this->wsClients as $clientID => $client ) {
			// if the client's opening handshake is complete, tell the client
	...
	function wsCheckIdleClients() {
		...
		foreach ( $this->wsClients as $clientID => $client ) {
			if ($client [2] != self::WS_READY_STATE_CLOSED) {
				...
				if ($client [4] !== false) {
					// ping request has already been sent to client, pending a
					// pong reply
					if ($time >= $client [4] + self::WS_TIMEOUT_PONG) {
						// client didn't respond to the server's ping request in
						// self::WS_TIMEOUT_PONG seconds
						$this->wsSendClientClose ( $clientID, self::WS_STATUS_TIMEOUT );
						$this->wsRemoveClient ( $clientID );
					}
				} elseif ($time >= $client [3] + self::WS_TIMEOUT_RECV) {
					// last data was received >= self::WS_TIMEOUT_RECV seconds
					// ago
					if ($client [2] != self::WS_READY_STATE_CONNECTING) {
	...
	function wsBuildClientFrame($clientID, &$buffer, $bufferLength) {
		...
		if ($this->wsClients [$clientID] [7] !== false || $this->wsCheckSizeClientFrame ( $clientID ) == true) {
			...
			if ($this->wsClients [$clientID] [8] >= $frameLength) {
				// check if too many bytes have been read for the frame (they
				// are part of the next frame)
		...
		if ($this->wsClients [$clientID] [8] > 1) {
			...
			if ($payloadLength <= 125) {
				...
				if (substr ( $this->wsClients [$clientID] [9], 9, 1 ) !== false) {
					...
					// 2,147,483,647 (31 bits)
					// the maximum integer in PHP is "usually" this number. More
					// info: http://php.net/manual/en/language.types.integer.php
					...
					// check if the payload data length exceeds 2,147,479,538
					// (2,147,483,647 - 14 - 4095)
			...
			if ($this->wsClients [$clientID] [7] !== false) {
				...
				// doesn't apply for control frames, where the payload data is
				// not internally stored
	...
	function wsProcessClientFrame($clientID) {
		...
		// check at least 6 bytes are set (first 2 bytes and 4 bytes for the
		// mask key)
		...
		// check if this is not a continuation frame and if there is already
		// data in the message buffer
